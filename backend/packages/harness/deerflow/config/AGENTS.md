...
- `tool_search.custom_provider` - Optional operator callable path (`module.path:function`) resolved once during agent assembly. It returns serializable app/connector descriptors for `tool_search`; descriptors never enter the MCP catalog/hash or execute during discovery, and the invocation tool remains subject to the normal lead/subagent policy.
