    ...
    *postgres_schema*, when set, is forwarded to the alembic config so the
    ...
    async with _bootstrap_lock(engine, backend=backend):
        ...
        if decision == "empty":
            ...
            # [argus patch #70] A version stamp can survive an incomplete/manual
            # migration. In that state Alembic considers the database current and
            # performs no DDL, while the DB run-event store fails later at first
            # write. Do not silently fall back or auto-create a table here: schema
