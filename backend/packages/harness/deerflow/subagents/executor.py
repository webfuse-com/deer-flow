    ...
    def execute_async(self, task: str, task_id: str | None = None) -> str:
        ...
        async def run_with_timeout() -> SubagentResult:
            try:
                ...
                # [argus patch #66] Salvage whatever the subagent managed to say
                # before the wall clock cut it off, exactly as the
                # GraphRecursionError path salvages a turn-capped run. Without
                # this the lead receives only "Task timed out." and every message
                # the subagent produced is dropped when cleanup_background_task
                # frees the buffer -- on thread 5a3be3f1 that discarded three
                # ~10-minute research runs and ~28M tokens of work. A capped run
                # with usable output is `completed` + `stop_reason`, which is how
                # the status contract already models token/turn/loop caps, so no
                # consumer needs to change to benefit.
