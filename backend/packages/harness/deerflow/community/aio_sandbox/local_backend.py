...
class LocalContainerBackend(SandboxBackend):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            container_network: [argus] patch #33. Container network to attach
                sandboxes to. When set (open network mode only), sandboxes join
                this network and are reached by container DNS name on port 8080
                with NO host port published; when None, upstream's host-port
                publish path is used.
        """
        self._image = image
        ...
        # [argus] patch #33: sandbox network-DNS mode, see _sandbox_url.
        self._container_network = (container_network or "").strip() or None
        if self._container_network and self._network_mode != "open":
            raise RuntimeError("sandbox.container_network (argus network-DNS mode) requires sandbox.network.mode 'open'; the restricted modes manage their own networks")
    ...
    def _sandbox_url(self, container_name: str, host_port: int | None) -> str:
        ...
        [argus] patch #33. Network-DNS mode: the gateway is on the same container
        network, so it resolves the sandbox by container name on the container's
        own port 8080 (no host port is published; the socket proxy refuses one).
        Otherwise upstream's host-publish form: DEER_FLOW_SANDBOX_HOST and the
        published host port.
        """
        if self._container_network:
    ...
    def create(
        ...
        *,
        ...
        if self._container_network:
            # [argus] patch #33: network-DNS mode publishes no host port, so
            # there is nothing to allocate and no "port already allocated"
            # failure mode. A container-name conflict (another process won the
            # race for this deterministic name) is handled by adopting it.
            try:
                container_id = self._start_container(
                    ...
                    network_override=self._container_network,
                    publish_port=False,
    ...
    def destroy(self, info: SandboxInfo) -> None:
        ...
        if self._container_network:
            # [argus] patch #33: no host port was allocated; the URL's 8080 is the
            # container's own port, not an entry in the port allocator.
            return
    ...
    def discover(self, sandbox_id: str) -> SandboxInfo | None:
        ...
        if self._container_network:
            # [argus] patch #33: reached by container DNS name; there is no host
            # port to look up (and none published).
            sandbox_url = self._sandbox_url(container_name, None)
        else:
    ...
    def list_running(self) -> list[SandboxInfo]:
        ...
        for container_name in container_names:
            ...
            elif not requires_replacement and host_port is None and not self._container_network:
            ...
            if requires_replacement:
                host_port = None
            if self._container_network and not requires_replacement:
                # [argus] patch #33: nothing is published, but orphan adoption
                # still needs a usable URL, so build the container-DNS one.
                sandbox_url = self._sandbox_url(container_name, None)
            else:
