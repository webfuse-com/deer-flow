...
class ViewImageMiddleware(AgentMiddleware[ViewImageMiddlewareState]):
    ...
    async def _describe_images_text(self, state: "ViewImageMiddlewareState") -> list[dict]:
        """[argus] Non-vision lead path: ask the vision model to describe each
        viewed image, return text content blocks for the injected HumanMessage."""
        ...
        blocks: list[dict] = [
            {
                "type": "text",
                "text": ("Here are descriptions of the images you viewed (you cannot see images directly; a vision model described them for you):"),
            }
        ]
        for image_path, image_data in viewed_images.items():
    ...
    async def _ainject_image_message(self, state: "ViewImageMiddlewareState") -> dict | None:
        """[argus] Async inject: text descriptions (non-vision lead) or raw image
        (vision lead). Internal context only, so hide from the chat UI/IM channels."""
        if not self._should_inject_image_message(state):
            return None
        ...
        return {"messages": [HumanMessage(content=content, additional_kwargs={"hide_from_ui": True})]}
    ...
    def before_model(self, state: ViewImageMiddlewareState, runtime: Runtime) -> dict | None:
        ...
        Returns:
            State update with additional human message, or None if no update needed
        """
        # [argus] The sync path cannot await the vision-describe call; the
        # lead-agent loop uses abefore_model (async). For a vision lead we
        # inject directly; for a non-vision lead, defer to abefore_model.
        if self._vision_model_name:
            return None
