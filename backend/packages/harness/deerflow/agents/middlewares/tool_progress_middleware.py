...
class ToolProgressMiddleware(AgentMiddleware[AgentState]):
    ...
    def __init__(
        ...
        *,
        ...
        # synchronous wrapper from multiple threads. Critical sections are short in-memory
        # dict ops with no I/O (classify_bash_command is hoisted outside the lock), and
        # recorder callbacks are deliberately invoked after this state lock is released;
        # observability must not stall tool state updates. This matches
        # LoopDetectionMiddleware's publication convention.
    ...
    def wrap_tool_call(
        ...
        if tool_name not in self._exempt_tools:
            result = self._update_state_from_result(result, tool_name, runtime, str(request.tool_call.get("id") or ""), hook="wrap_tool_call")
        self._record_run_efficiency(result, tool_name, runtime, tool_args=tool_args)
        return result
    ...
    async def awrap_tool_call(
        ...
        if tool_name not in self._exempt_tools:
            result = self._update_state_from_result(result, tool_name, runtime, str(request.tool_call.get("id") or ""), hook="awrap_tool_call")
        self._record_run_efficiency(result, tool_name, runtime, tool_args=tool_args)
        return result
