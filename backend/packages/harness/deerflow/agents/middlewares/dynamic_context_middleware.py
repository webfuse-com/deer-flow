...
class DynamicContextMiddleware(AgentMiddleware):
    ...
    def _build_full_reminder(self, runtime: Runtime | None = None, *, query: str | None = None, inject_memory: bool = True) -> tuple[str, str | None]:
        ...
        injection_enabled = self._memory_enabled and inject_memory and (self._app_config.memory.injection_enabled if self._app_config else True)
        memory_context = (
            _get_memory_context(
                self._agent_name,
    ...
    def _inject(self, state, runtime: Runtime | None = None) -> dict | None:
        ...
        if last_date is None:
            ...
            date_reminder, memory_block = self._build_full_reminder(
                runtime,
                query=_derive_injection_query(messages[target_idx]),
                inject_memory=effective_memory_mode(runtime) != "off",
            )
            logger.info(
