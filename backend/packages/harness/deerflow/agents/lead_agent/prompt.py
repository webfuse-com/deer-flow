...
def _build_subagent_section(
    ...
    *,
    ...
    # [argus patch #64] parallel_first only re-frames the guidance when parallel
    # dispatch is actually possible; with a per-response limit of 1 the
    # conservative framing stays (there is nothing to parallelize).
...
- `does not hold`: inspect the recorded reason, repair or recheck the unmet condition, and reuse unaffected outputs. If another delegation is worthwhile, name the missing condition and scope it only to that gap.
- `UNVERIFIED`: this is missing evidence, not a failed condition. Verify load-bearing criteria against actual artifacts or primary evidence; if confirmation is unavailable, preserve uncertainty in the answer instead of upgrading it to a checked fact.
- `holds`: reuse the checked outputs; the check proves only the stated execution condition. Still spot-check load-bearing claims beyond its scope. With no checklist, inspect the self-report and its handles directly.
