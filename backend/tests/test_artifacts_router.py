...
def test_get_artifact_serves_active_content_inline_inside_a_csp_sandbox(tmp_path, monkeypatch, filename: str, content: str) -> None:
    ...
    # Upstream #4864: the inline branch carries a real SHA-256 ETag so the
    # frontend can enable inline editing without crypto.subtle.
...
def test_get_artifact_large_active_content_skips_etag(tmp_path, monkeypatch) -> None:
    # Active content (e.g. .html) renders inline inside a CSP sandbox (argus
    # patch #89). A large active file must still do so but skip the full-file
    # SHA-256 pass (performance P1).
