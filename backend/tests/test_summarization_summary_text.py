...
class TestSummaryWritesChannel:
    ...
    async def test_rescued_user_does_not_drop_earlier_tool_exchanges(self, async_mode):
        ...
        # [argus patch #75] the rescued request is handed to the summarizer in
        # its own <active_user_request> block; it must never read as history.
    ...
    async def test_mixed_history_empty_trim_preserves_recent_tool_result(self, async_mode):
        ...
        # [argus patch #75] see test_rescued_user_does_not_drop_earlier_tool_exchanges
        assert "CURRENT_REQUEST" not in new_text
    ...
    def test_preserved_active_user_request_is_included_in_summary_prompt(self):
        ...
        # The request travels in its own block, ahead of the history sections.
        assert model.prompts[-1].index("<active_user_request>") < model.prompts[-1].index("<new_messages>")
