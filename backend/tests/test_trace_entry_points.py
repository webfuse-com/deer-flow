...
async def test_inbound_messages_are_handled_under_distinct_trace_scopes(tmp_path: Path):
    ...
    manager = ChannelManager(
        ...
        max_concurrency=1,
        # [argus patch coalesce] split-paste coalescing would merge these two
        # messages from one chat into a single unit of work and the loop below
        # would never see two handled messages. The window is disabled here for
        # the same reason the other channel suites disable it: this test is
        # about trace scoping, not intake batching.
        coalesce_window=0,
    )
    seen: list[str | None] = []
