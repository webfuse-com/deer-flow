...
# ── [argus] patch #33: container_network reaches the local backend ───────────


def test_load_config_maps_legacy_network_string_to_container_network(monkeypatch):
    """Every stack config writes ``sandbox.network: <name>``. Upstream's ``network``
    key is now the egress-policy object, so the string form maps onto
    ``container_network`` and the policy object keeps its defaults."""
    aio_mod = importlib.import_module("deerflow.community.aio_sandbox.aio_sandbox_provider")
    sandbox_config = SandboxConfig(use="deerflow.community.aio_sandbox:AioSandboxProvider", network="argus-x-sandbox-net")
    app_config = SimpleNamespace(sandbox=sandbox_config, stream_bridge=None)
    monkeypatch.setattr(aio_mod, "get_app_config", lambda: app_config)
    provider = aio_mod.AioSandboxProvider.__new__(aio_mod.AioSandboxProvider)
    ...
    assert cfg["container_network"] == "argus-x-sandbox-net"
    assert cfg["network"]["mode"] == "open"


def test_load_config_keeps_upstream_network_object_and_no_container_network(monkeypatch):
    aio_mod = importlib.import_module("deerflow.community.aio_sandbox.aio_sandbox_provider")
    sandbox_config = SandboxConfig(use="deerflow.community.aio_sandbox:AioSandboxProvider", network={"mode": "isolated"})
    app_config = SimpleNamespace(sandbox=sandbox_config, stream_bridge=None)
    monkeypatch.setattr(aio_mod, "get_app_config", lambda: app_config)
    provider = aio_mod.AioSandboxProvider.__new__(aio_mod.AioSandboxProvider)
    ...
    assert cfg["container_network"] is None
    assert cfg["network"]["mode"] == "isolated"


def test_create_backend_threads_container_network_to_local_backend(tmp_path, monkeypatch):
    aio_mod = importlib.import_module("deerflow.community.aio_sandbox.aio_sandbox_provider")
    provider = _make_provider(tmp_path)
    provider._config = {
        "image": "img:latest",
        ...
        "provisioner_url": "",
        "network": {"mode": "open"},
        "container_network": "argus-x-sandbox-net",
    }
    captured = {}
    ...
    assert captured.get("container_network") == "argus-x-sandbox-net"
    assert captured.get("network_config") == {"mode": "open"}
