...
def test_list_running_network_mode_builds_dns_urls(monkeypatch):
    """[argus] patch #33: in network-DNS mode containers publish no host port, but
    list_running must still yield a usable container-DNS URL so orphan adoption
    works (a "" URL would build a broken AioSandbox client), and an unpublished
    port must not mark the sandbox as needing replacement."""
    from deerflow.community.aio_sandbox.local_backend import LocalContainerBackend
    ...
    assert infos[0].requires_replacement is False
