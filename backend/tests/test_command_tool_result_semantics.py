...
def test_command_error_receipt_is_error_not_success():
    ...
    # [argus patch #73] Upstream leaves LangChain's public status at the
    # producer's "success" here and carries the truth only in the meta and the
    # receipt. This fork syncs the public field to the meta
    # (`tool_result_meta._sync_message_status`) because ReadBeforeWriteMiddleware
    # reads `message.status` to decide whether a write actually landed; a bash
    # write that exits non-zero must not refresh the read mark.
...
def test_setup_agent_empty_soul_receipt_is_error():
    ...
    assert message.status == "error"  # [argus patch #73] public status follows the meta
    assert _meta(message)["status"] == "error"
    assert message.additional_kwargs[TOOL_RECEIPT_KEY]["status"] == "error"
