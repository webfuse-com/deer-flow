...
# ── [argus] patch #33: sandbox network-DNS mode ──────────────────────────────
# With ``container_network`` set, sandboxes join that container network and the
# gateway reaches them by container DNS name on port 8080; no host port is
# published (the Argus socket proxy refuses PortBindings, and rootless Podman
# host-port binds raced). Unset keeps upstream's host-publish path byte for byte.


def _dns_backend() -> LocalContainerBackend:
    backend = LocalContainerBackend(
        image="sandbox:latest",
        base_port=8080,
        container_prefix="sandbox",
        config_mounts=[],
        environment={},
        container_network="argus-net",
    )
    backend._runtime = "docker"
    return backend


def _capture_create_command(monkeypatch, backend: LocalContainerBackend):
    captured_cmd: list[str] = []
    ...
    return captured_cmd, info


def _unlabelled_inspection(host_port=None):
    from deerflow.community.aio_sandbox.local_backend import _ContainerInspection

    return _ContainerInspection(created_at=1.0, host_port=host_port, labels={}, image="sandbox:latest", networks=frozenset({"argus-net"}))


def test_create_network_dns_mode_joins_network_and_publishes_no_port(monkeypatch):
    _clear_hardening_env(monkeypatch)
    backend = _dns_backend()

    def _boom(*args, **kwargs):
        raise AssertionError("get_free_port must not be called in network-DNS mode")

    monkeypatch.setattr("deerflow.community.aio_sandbox.local_backend.get_free_port", _boom)

    cmd, info = _capture_create_command(monkeypatch, backend)

    assert cmd[cmd.index("--network") + 1] == "argus-net"
    assert "-p" not in cmd
    assert cmd[cmd.index("--name") + 1] == "sandbox-abc12345"
    assert "deerflow.sandbox_id=abc12345" in cmd  # upstream's identity labels still apply
    assert info.container_name == "sandbox-abc12345"
    assert info.sandbox_url == "http://sandbox-abc12345:8080"


def test_create_without_container_network_keeps_upstream_host_publish(monkeypatch):
    _clear_hardening_env(monkeypatch)
    backend = _backend_for_inspect_tests()
    monkeypatch.setenv("DEER_FLOW_SANDBOX_HOST", "host.docker.internal")
    monkeypatch.setattr("deerflow.community.aio_sandbox.local_backend.get_free_port", lambda start_port: 18080)
    monkeypatch.setattr("deerflow.community.aio_sandbox.local_backend._resolve_docker_bind_host", lambda: "127.0.0.1")

    cmd, info = _capture_create_command(monkeypatch, backend)

    assert cmd[cmd.index("-p") + 1] == "127.0.0.1:18080:8080"
    assert "--network" not in cmd
    assert info.sandbox_url == "http://host.docker.internal:18080"


def test_create_network_dns_mode_adopts_existing_container_on_name_conflict(monkeypatch):
    backend = _dns_backend()

    def fake_start(*args, **kwargs):
        raise RuntimeError('Error response from daemon: Conflict. The container name "/sandbox-abc12345" is already in use by container "deadbeef".')

    monkeypatch.setattr(backend, "_start_container", fake_start)
    existing = SandboxInfo(sandbox_id="abc12345", sandbox_url="http://sandbox-abc12345:8080", container_name="sandbox-abc12345")
    monkeypatch.setattr(backend, "discover", lambda sandbox_id: existing)

    assert backend.create(thread_id=None, sandbox_id="abc12345") is existing


def test_discover_network_dns_mode_needs_no_host_port(monkeypatch):
    """A pre-sync (unlabelled, unpublished) sandbox is a legacy open sandbox and
    stays adoptable by DNS name, so a rollout does not strand running threads."""
    backend = _dns_backend()
    monkeypatch.setattr(backend, "_is_container_running", lambda name: True)
    monkeypatch.setattr(backend, "_batch_inspect", lambda names, strict=False: {"sandbox-abc12345": _unlabelled_inspection()})

    def _boom(*args, **kwargs):
        raise AssertionError("_get_container_port must not be called in network-DNS mode")

    monkeypatch.setattr(backend, "_get_container_port", _boom)
    seen: dict[str, str] = {}

    def fake_ready(url, timeout=5, **kwargs):
        seen["url"] = url
        return True
    ...
    assert info is not None and not info.requires_replacement
    assert info.sandbox_url == "http://sandbox-abc12345:8080" == seen["url"]


def test_destroy_network_dns_mode_releases_no_port(monkeypatch):
    backend = _dns_backend()
    monkeypatch.setattr(backend, "_stop_container", lambda target: None)

    def _boom(port):
        raise AssertionError("release_port must not be called in network-DNS mode")

    monkeypatch.setattr("deerflow.community.aio_sandbox.local_backend.release_port", _boom)

    backend.destroy(SandboxInfo(sandbox_id="x", sandbox_url="http://sandbox-x:8080", container_name="sandbox-x", container_id="cid"))


def test_container_network_requires_open_network_mode():
    with pytest.raises(RuntimeError, match="requires sandbox.network.mode 'open'"):
        LocalContainerBackend(
            image="sandbox:latest",
            base_port=8080,
            container_prefix="sandbox",
