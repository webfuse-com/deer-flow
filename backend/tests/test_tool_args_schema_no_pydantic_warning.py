...
_SANDBOX_TOOL_CASES = [
    ...
    (
        # [argus patch #72] old_str/new_str are optional because a ``replacements``
        # batch may stand in for them; the model-facing contract keeps both forms.
        str_replace_tool,
        {"path": "/tmp/x", "old_str": "a", "new_str": "b"},
        ("path",),
        ("path", "old_str", "new_str", "description", "replace_all", "replacements"),
    ),
]
...
_SANDBOX_TOOL_FORWARDING_CASES = [
    ...
    (
        ...
        # [argus patch #72] the async wrapper also forwards ``replacements``.
        ("/path", "old", "new", "description", True, None),
    ),
]
