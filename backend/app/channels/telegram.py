...
class TelegramChannel(Channel):
    ...
    async def send(self, msg: OutboundMessage, *, _max_retries: int = 3) -> None:
        ...
        # _max_retries kwarg — matches upstream. The stream-edit/rich helpers
    ...
    async def _upstream_send_superseded(self, msg: OutboundMessage, *, _max_retries: int = 3) -> None:
    ...
    async def _upstream_send_running_reply_superseded(self, chat_id: str, reply_to_message_id: int) -> None:
        ...
        verbatim but unreachable (the live path is the stage-emoji indicator in
        _telegram_sender). TestTelegramStreaming pins this copy so upstream
        churn here merges clean."""
        if not self._application:
            return
        try:
            bot = self._application.bot
    ...
    async def _send_running_reply(self, chat_id: str, reply_to_message_id: int) -> None:
        """Show the 'received' stage indicator for an accepted inbound message."""
    ...
    async def _process_incoming_with_reply(
        ...
        *,
        ...
        try:
            # [argus patch #27] Fire the received emoji in the background so
            # the inbound message reaches the agent pipeline immediately; the
            # indicator is UI-only and self-recovers via the reaction fallback.
