...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if internal_user is not None:
            ...
            # [argus patch #15] Trusted-proxy SSO identity (resolved above). Sits
            # behind the Bearer branch on purpose: a present Authorization header
            # is authoritative upstream-wide and never falls back to a browser
            # identity, and the SSO path is a browser identity like the cookie.
            user = sso_user
        elif access_token:
