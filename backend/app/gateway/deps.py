...
async def resolve_or_provision_sso_user(email: str):
    """Resolve a DeerFlow user for an edge-verified SSO email, creating the
    record on first sight (auto-provision). Used by the trusted-proxy SSO path
    (sso_auth.trusted_sso_email) so a citizen who already passed Google SSO at
    the Caddy edge is not asked to log in again. The email is trusted because it
    arrived with the Caddy proxy secret (constant-time verified upstream)."""
    provider = get_local_provider()
