...
async def acquire_debug_sandbox(thread_id: str, request: Request) -> DebugSandboxResponse:
    ...
    # Container-backed sandbox ids are the deterministic 8-char hex hash that
    # also names the container. Filesystem-local providers return "local" or
    # "local:<thread_id>" — those have no container to proxy to, so the debug
    # link is not available for them.
