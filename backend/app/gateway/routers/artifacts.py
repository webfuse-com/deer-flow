...
# Argus patch #89: what a browser can display when a citizen opens an artifact
# in a new tab. Chrome only renders these ``text/*`` types inline; every other
# text type (``text/markdown``, ``text/csv``, ``text/x-python``, ...) is offered
...
def _inline_media_type(mime_type: str | None) -> str | None:
    """The media type a browser can show inline for ``mime_type``."""
    if mime_type and mime_type.startswith("text/") and mime_type not in INLINE_TEXT_MIME_TYPES:
...
def _read_artifact_payload(actual_path: Path, path: str, download: bool) -> tuple[str, str | None, dict[str, str]]:
    ...
    ``(kind, media_type, extra_headers)`` plan the handler turns into a streamed
    ...
    # An explicit download is streamed by FileResponse as an attachment; no read here.
    if download:
    ...
    if is_text_file_by_content(actual_path):
        return ("inline_file", mime_type or INLINE_TEXT_MEDIA_TYPE, {})
...
@router.get(
    ...
    description="Retrieve an artifact file generated by the AI agent. Files a browser can show are served inline (active web content inside a CSP sandbox); download=true returns an attachment.",
)
...
async def get_artifact(thread_id: ThreadId, path: str, request: Request, download: bool = False) -> Response:
    ...
    Example:
        ...
        - Open a generated page: `/api/threads/abc123/artifacts/mnt/user-data/outputs/index.html` (sandboxed)
    """
    ...
    if ".skill/" in path:
        ...
        if mime_type and mime_type.startswith("text/"):
            return Response(content=ranged_content, status_code=status_code, media_type=_inline_media_type(mime_type), headers=inline_headers)
    ...
    kind, mime_type, extra_headers = await asyncio.to_thread(_read_artifact_payload, actual_path, path, download)

    if kind == "file":
        # ``download=true``: an attachment under the real media type, so the
        # saved file keeps its name and type (patch #89).
