...
3. The bot accepts inbound text, photos, documents, and voice notes (with or without captions). Voice notes are transcribed by an optional overlay STT hook and injected as text; if that hook is missing or fails, the bot replies that it could not transcribe and does not start a turn. Hosted Bot API downloads are limited to 20 MB per attachment.
...
For implementation-heavy runs, `str_replace` accepts an ordered `replacements`
batch for one atomic same-file edit. Operators can also configure soft
ToolProgress read-only and total-call budgets that prompt a phase transition
without lowering recursion limits or stopping a legitimate long build.
...
For tool-heavy deployments, DeerFlow also supports prompt and execution efficiency controls in `config.yaml`: deferred skill discovery with bounded high-confidence auto-routing, adaptive reasoning that uses a no-thinking model variant after successful deterministic tools, wall-clock plus model-call run limits, and hash-guarded `workspace_inspect`/`workspace_patch` batch operations. Injected skill bodies are treated as already loaded, and an externalized `workspace_inspect` result keeps every requested file's success/error outcome in its inline synopsis so a large batch does not trigger redundant discovery. All controls are opt-in; see `config.example.yaml` for the settings and safe defaults.
...
The Argus fork keeps browser tools, API endpoints, streamed frames, and inline browser previews available, but omits the Browser Live header trigger from its shared workspace UI. An explicit Custom Agent tool-group allowlist without `browser` still prevents that agent from receiving browser tools.
